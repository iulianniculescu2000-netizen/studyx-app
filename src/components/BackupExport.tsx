      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed z-[201] left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm rounded-2xl p-6 shadow-2xl"
            style={{ background: theme.modalBg, border: `1px solid ${theme.border}` }}>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-lg font-bold" style={{ color: theme.text }}>Backup & Export</h2>
                <p className="text-xs mt-0.5" style={{ color: theme.text3 }}>Exportă sau restaurează toate datele tale</p>
              </div>
            ...
            <div className="space-y-3">
              <motion.button whileTap={{ scale: 0.97 }} onClick={exportBackup}
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: theme.text }}>Exportă backup</p>
                  <p className="text-xs" style={{ color: theme.text3 }}>Grile, statistici, notițe — tot</p>
                </div>
              </motion.button>

              <motion.button whileTap={{ scale: 0.97 }} onClick={importBackup}
                className="w-full flex items-center gap-3 p-4 rounded-2xl text-left"
                style={{ background: `${theme.success}10`, border: `1px solid ${theme.success}30` }}>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                  style={{ background: `${theme.success}20` }}>
                  <Upload size={18} style={{ color: theme.success }} />
                </div>
                <div>
                  <p className="text-sm font-semibold" style={{ color: theme.text }}>Importă backup</p>
                  <p className="text-xs" style={{ color: theme.text3 }}>Restaurează din fișier JSON</p>
                </div>
              </motion.button>
            </div>
